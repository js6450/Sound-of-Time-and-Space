...
var sentH, sentS, sentB;
var prevH, prevS, prevB;
...
function setup(){
    ...
    //test echoes
    socket.on('echoSend', onEchoSend);
    socket.on('echoPrevious', onEchoPrevious);
    ...
    interval = Math.floor(Math.random() * 2500) + 500;
}
...
function draw(){
    ...
    rect(width - 50, 100, 100, 200);
    //sent
    fill(sentH, sentS, sentB);
    rect(width - 150, 50, 100, 100);
    //previous
    fill(prevH, prevS, prevB);
    rect(width - 150, 150, 100, 100);

    fill(255);
    noStroke();
    text("now", width - textWidth("now") * 3 / 2, 24);
    text("sent", width - textWidth("sent") * 3 / 2 - 100, 24);
    text("prev", width - textWidth("prev") * 3 / 2 - 100, 24 + 100);
...
function onEchoSend(data){
    sentH = parseInt(data.substr(0, 3));
    sentS = map(parseInt(data.substr(3, 1)), 0, 9, 50, 100);
    sentB = map(parseInt(data.substr(4, 1)), 0, 9, 50, 100);
}

function onEchoPrevious(data){
    prevH = parseInt(data.substr(0, 3));
    prevS = map(parseInt(data.substr(3, 1)), 0, 9, 50, 100);
    prevB = map(parseInt(data.substr(4, 1)), 0, 9, 50, 100);
}
...
function mode_test_display(){
    ...
    if(clickTime - lastClicked >= interval){
        colorS = 0;

        var x = Math.floor(map(random(0, width), 0, width, 0, 99));
        var y = Math.floor(map(random(0, height), 0, height, 0, 99));
